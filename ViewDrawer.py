...
class ViewDrawer:
    ...
    def addImage(name, frame, pos, parent):
        ...
        try:
            with dpg.texture_registry(show=False):
                dpg.add_raw_texture(
                    frame.shape[1], frame.shape[0], texture_data, tag=name, format=dpg.mvFormat_Float_rgb)
        except SystemError:
    ...
    def drawHLinks(self):
        for i in range(0, self.dataset.rowsCount() - 1):
            for j in range(0, self.dataset.columnsCount()):
                if self.dataset.getStitchResult([i, j], [i + 1, j]):
                    color = (0, 255, 0)
                else:
                    color = (255, 0, 0)
    ...
    def drawVLinks(self):
        for i in range(0, self.dataset.rowsCount()):
            for j in range(0, self.dataset.columnsCount() - 1):
                if self.dataset.getStitchResult([i, j], [i, j + 1]):
                    color = (0, 255, 0)
                else:
                    color = (255, 0, 0)
