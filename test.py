...
def processLine(path):
    ...
    finalResults = None
    prevX, prevY = 0, 0
    for i in range(1, dataset.columnsCount()):
        print(f'\nProcessed: {i - 1} - > {i}')
        prev = dataset.at(0, i - 1).getImage()
        current = dataset.at(0, i).getImage()
    ...
    cv2.imwrite('finalBlended.png', blended[0])
